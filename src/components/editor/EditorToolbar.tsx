...
export function EditorToolbar({
    ...
    /* Dismounts active dropdown menus upon detecting external clicks. */
    useEffect(() => {
        ...
        <div style={headerStyle}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', position: 'relative' }} ref={menuRef}>
                ...
                <div style={{ position: 'relative' }}>
                    <button 
                        style={{ ...menuButtonStyle, background: activeMenu === 'file' ? '#333' : 'transparent' }} 
                    ...
                    {activeMenu === 'file' && (
                        <div style={dropdownStyle}>
                            <button 
                                ...
                                onClick={() => { handleSave(true); setActiveMenu(null); }}
                                data-testid="save-project-btn"
                            >
                                <Save size={14} color="#e0e0e0" /> Save Project
                            </button>
                            <button 
                                style={menuButtonStyle} 
                                onClick={() => { fileInputRef.current?.click(); }}
                            >
                                <FolderOpen size={14} color="#e0e0e0" /> Load Project
                            </button>
                        </div>
                    )}
                    <input 
                ...
                <div style={{ position: 'relative' }}>
                    <button 
                        style={{ ...menuButtonStyle, background: activeMenu === 'settings' ? '#333' : 'transparent' }} 
                    ...
                    {activeMenu === 'settings' && (
                        <div style={{ ...dropdownStyle, width: '220px', padding: '12px' }}>
                            <div style={{ fontSize: '10px', color: '#888', fontWeight: 'bold', marginBottom: '8px' }}>
                                {activeContext.name.toUpperCase()} SETTINGS
                            </div>
                ...
                <button style={menuButtonStyle} onClick={onOpenConfigMenu}>Config</button>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    <input 
                        ...
                        placeholder="mod_id" 
                        style={{ ...compactInputStyle, color: '#4dabf7', width: '80px' }} 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    <input 
                        ...
                        style={{ ...compactInputStyle, fontWeight: 'bold', width: '120px' }} 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <select 
                    ...
                    onChange={(e) => { 
                        onFlowChange(nodes, edges, graph, history.past, history.future); 
